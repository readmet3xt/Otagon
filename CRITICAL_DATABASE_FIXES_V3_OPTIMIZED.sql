-- ========================================
-- CRITICAL DATABASE FIXES V3 - OPTIMIZED
...
-- 3. Clean up existing policies first
DROP POLICY IF EXISTS "Allow authenticated users to read app_level" ON public.app_level;
DROP POLICY IF EXISTS "Allow authenticated users to modify app_level" ON public.app_level;
DROP POLICY IF EXISTS "Anyone can view app level" ON public.app_level;
DROP POLICY IF EXISTS "Users can access their own diary tasks" ON public.diary_tasks;
DROP POLICY IF EXISTS "Users can manage their own diary tasks" ON public.diary_tasks;
DROP POLICY IF EXISTS "Allow authenticated users to access games" ON public.games;
DROP POLICY IF EXISTS "Anyone can view games" ON public.games;
DROP POLICY IF EXISTS "Users can access their own usage data" ON public.user_usage;

-- 4. Fix app_level table access issues
...
-- Add RLS policies for app_level table (optimized)
ALTER TABLE public.app_level ENABLE ROW LEVEL SECURITY;

-- Single optimized policy for app_level
CREATE POLICY "app_level_access" ON public.app_level
    FOR ALL USING ((SELECT auth.role()) = 'authenticated');

-- 5. Fix conversations table queries
-- Ensure conversations table has proper structure
ALTER TABLE public.conversations 
ADD COLUMN IF NOT EXISTS ai_data JSONB DEFAULT '{}'::jsonb;

-- 6. Fix diary_tasks table
...
-- Add RLS policies for diary_tasks (optimized)
ALTER TABLE public.diary_tasks ENABLE ROW LEVEL SECURITY;

-- Single optimized policy for diary_tasks
CREATE POLICY "diary_tasks_access" ON public.diary_tasks
    FOR ALL USING (user_id = (SELECT auth.uid()));

-- 7. Fix games table queries
...
-- Add RLS policies for games (optimized)
ALTER TABLE public.games ENABLE ROW LEVEL SECURITY;

-- Single optimized policy for games
CREATE POLICY "games_access" ON public.games
    FOR ALL USING ((SELECT auth.role()) = 'authenticated');

-- 8. Fix user_usage table structure
...
-- Add RLS policies for user_usage (optimized)
ALTER TABLE public.user_usage ENABLE ROW LEVEL SECURITY;

-- Single optimized policy for user_usage
CREATE POLICY "user_usage_access" ON public.user_usage
    FOR ALL USING (user_id = (SELECT auth.uid()));
...
-- 11. Insert default app_level data
