...
export class SubtabsService {
  ...
  async addSubtab(conversationId: string, subtab: SubTab): Promise<SubTab | null> {
    if (FEATURE_FLAGS.USE_NORMALIZED_SUBTABS) {
      return this.addSubtabToTable(conversationId, subtab);
    } else {
  ...
  async updateSubtab(
    ...
    if (FEATURE_FLAGS.USE_NORMALIZED_SUBTABS) {
      return this.updateSubtabInTable(subtabId, updates);
    } else {
  ...
  private async setSubtabsInTable(
    ...
    try {
      ...
      // @ts-expect-error - conversation_id column will exist after migration
  ...
  private async addSubtabToTable(
    ...
    try {
      ...
      return {
        ...
        isNew: data.metadata?.isNew || false,
        status: data.metadata?.status || 'loaded',
  ...
  private async updateSubtabInTable(
    ...
    try {
      ...
      // @ts-expect-error - types will be correct after migration and regeneration
