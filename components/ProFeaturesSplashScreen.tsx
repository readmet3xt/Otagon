...
const ProFeature: React.FC<{ title: string; description: string; children: React.ReactNode; }> = ({ title, description, children }) => (
    <div className="flex items-start gap-6 sm:gap-8 p-6 bg-gradient-to-r from-[#1C1C1C]/80 to-[#0A0A0A]/80 backdrop-blur-xl rounded-2xl border-2 border-neutral-800/60 hover:border-neutral-700/80 hover:scale-105 transition-all duration-500 feature-card">
...
const proFeatures = [
    ...
    { title: "Batch Screenshot Capture", description: "Instantly capture the last 5 minutes of gameplay with a hotkey. Otakon analyzes all key moments for a comprehensive summary.", icon: <KeyboardIcon className="w-6 h-6 text-[#FFAB40]" /> },
...
const vanguardFeatures = [
    ...
    { title: "Direct Influence", description: "Join the Founder's Council on Discord to help shape the future of Otakon.", icon: <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 text-amber-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></svg> },
...
const ProFeaturesSplashScreen: React.FC<ProFeaturesSplashScreenProps> = ({ onComplete, onUpgrade, onUpgradeToVanguard }) => {
    ...
    return (
        <div className="h-screen bg-gradient-to-br from-[#111111] to-[#0A0A0A] text-[#F5F5F5] flex flex-col font-inter pro-features-mobile">
            {/* Sticky Header */}
            <header className="sticky top-0 bg-gradient-to-b from-[#111111]/95 to-[#0A0A0A]/95 backdrop-blur-sm border-b border-[#424242]/20 px-4 sm:px-6 md:px-8 pt-4 sm:pt-6 md:pt-8 pb-2 sm:pb-3 md:pb-4 text-center z-10">
                <div className="max-w-2xl w-full mx-auto">
                    <div className="flex justify-center mb-1">
                        <StarIcon className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 text-[#FFAB40]" />
                    </div>
                    <h1 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-[#FF4D4D] to-[#FFAB40] leading-normal">Supercharge with Otagon Pro</h1>
                    <p className="text-base text-neutral-300 mb-4 leading-relaxed mt-2">Unlock exclusive features for the ultimate gaming companion.</p>
            ...
            <footer className="flex-shrink-0 w-full sticky bottom-0 bg-gradient-to-b from-transparent via-[#0A0A0A]/80 to-[#0A0A0A]/95 backdrop-blur-md border-t border-[#424242]/30 px-4 sm:px-6 md:px-8 pt-4 sm:pt-6 md:pt-8 pb-[calc(env(safe-area-inset-bottom)+16px)] sm:pb-[calc(env(safe-area-inset-bottom)+24px)] md:pb-[calc(env(safe-area-inset-bottom)+32px)] z-10 shadow-2xl shadow-black/50">
                <div className="w-full max-w-lg mx-auto space-y-3 sm:space-y-4">
                    ...
                    <button
                        onClick={onComplete}
                        className="w-full text-neutral-300 font-medium py-2 px-4 rounded-xl hover:bg-neutral-800/50 transition-all duration-300 hover:scale-105 text-sm flex items-center justify-center"
                    >
                        Maybe Later
                    </button>
                </div>
