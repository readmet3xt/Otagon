...
import { developerModeDataService } from './developerModeDataService';
...
export interface UnifiedDataResult<T> {
  data: T;
  source: 'supabase' | 'localStorage' | 'fallback' | 'developer_mode';
...
export class UnifiedDataService {
   ...
   * For developer mode: Developer mode data service (exact Supabase schema)
   */
  async getData<T>(
    key: string,
    ...
    try {
      ...
      // For developer mode: Use developer mode data service with exact Supabase schema
      if (this.isDeveloperMode) {
        // Use the developer mode data service which has exact Supabase schema structure
        const devData = await this.getDeveloperModeData(key, defaultValue);
        return {
          data: devData,
  ...
  async setData<T>(
    ...
    try {
      ...
      // For developer mode: Use developer mode data service with exact Supabase schema
      if (this.isDeveloperMode) {
        const success = await this.setDeveloperModeData(key, value);
        return {
          data: undefined as any,
          source: 'developer_mode',
          authenticated: false
  ...
  // Helper methods for developer mode data operations
  private async getDeveloperModeData<T>(key: string, defaultValue: T): Promise<T> {
    try {
      // Map common keys to developer mode data service methods
      switch (key) {
        case 'user_data':
          return await developerModeDataService.getUserData() as T;
        case 'conversations':
          return await developerModeDataService.getConversations() as T;
        case 'games':
          return await developerModeDataService.getGames() as T;
        case 'tasks':
          return await developerModeDataService.getTasks() as T;
        case 'analytics':
          return await developerModeDataService.getAnalytics() as T;
        default:
          // For cache or other keys, use cache methods
          const cacheData = await developerModeDataService.getCache(key);
          return cacheData || defaultValue;
      }
    } catch (error) {
      console.error(`Failed to get developer mode data for ${key}:`, error);
      return defaultValue;
    }
  }

  private async setDeveloperModeData<T>(key: string, value: T): Promise<boolean> {
    try {
      // Map common keys to developer mode data service methods
      switch (key) {
        case 'user_data':
          return await developerModeDataService.updateUserData(value as any);
        case 'conversations':
          // For conversations, we need to handle individual conversation updates
          // This is a simplified version - in practice, you'd want more specific methods
          return await developerModeDataService.setCache(key, value);
        case 'games':
          // For games, we need to handle individual game updates
          return await developerModeDataService.setCache(key, value);
        case 'tasks':
          // For tasks, we need to handle individual task updates
          return await developerModeDataService.setCache(key, value);
        case 'analytics':
          // For analytics, we need to handle individual analytics updates
          return await developerModeDataService.setCache(key, value);
        default:
          // For cache or other keys, use cache methods
          return await developerModeDataService.setCache(key, value);
      ...
      console.error(`Failed to set developer mode data for ${key}:`, error);
      return false;
    }
  }
