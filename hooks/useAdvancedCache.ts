...
import { CacheStrategy, CachePerformanceMetrics } from '../services/types';
...
export function useAdvancedCache<T = any>(options: UseAdvancedCacheOptions<T>): UseAdvancedCacheResult<T> {
  ...
  const loadFromCache = useCallback(async () => {
    try {
      ...
      const cachedData = await unifiedCacheService().get<T>(key, strategy);
  ...
  const update = useCallback(async (value: T) => {
    try {
      await unifiedCacheService().set(key, value, strategy);
      setData(value);
      setLastUpdated(new Date());
  ...
  const getCacheInfo = useCallback(() => {
    return unifiedCacheService().getCacheInfo();
  }, []);
