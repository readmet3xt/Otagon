...
export interface EnhancedInsight {
    ...
    generationModel: 'free' | 'flash' | 'pro';
    lastUpdated: number;
...
export const useEnhancedInsights = (
    ...
    useEffect(() => {
        if (profile) {
            (async () => {
                try {
                    const tabs = unifiedAIService().getTabsForGenre(genre || 'default');
                    let prioritizedTabs = tabs;
                    if (profile) {
                        const resolvedProfile = await profile;
                        if (resolvedProfile) {
                            prioritizedTabs = unifiedAIService().prioritizeTabsForProfile(tabs, resolvedProfile);
                        }
                    }
                    setInsightTabs(prioritizedTabs);
                
                    // Check if this is a new game pill that needs Pro model generation
                    const needsGeneration = unifiedAIService().needsContentGeneration(prioritizedTabs);
                    setIsNewGamePill(needsGeneration);
                    
                    // Check if any tabs need Pro model (only for paid users with new game pills)
                    const proTabs = unifiedAIService().getTabsForProModel(prioritizedTabs, 'paid');
    ...
    const generateInsightsForNewGame = useCallback(async (
        ...
        try {
            ...
            const results = await unifiedAIService().generateInsightsForNewGamePill(
                gameName, 
                genre, 
                progress, 
                userTier,
    ...
    const updateInsightsForUserQuery = useCallback(async (
        ...
        try {
            ...
            const resolvedProfile = await profile;
            if (!resolvedProfile) return;
            const results = await unifiedAIService().updateInsightsForUserQuery(
                gameName, 
                genre, 
                progress, 
                conversationId,
